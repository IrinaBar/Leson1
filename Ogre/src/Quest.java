...
public class Quest {
    ...
    private static final int ANIMATION_INTERVAL = 5_000; //static heißt,dass die Methode an der Klasse hängen und nicht am Object
     ...
     */
    public Quest() {
        this.adventurer = new Ogre(OGRE_X - OGRE_DIST, OGRE_Y, "green", OGRE_INITIAL_LIFEPOINTS, OGRE_INITIAL_STRENGTH);
    }
    ...
    void run() {
        ...
        for (int index = 0; index < path.length; index++) {
            ...
            case MEET:
                ...
                Ogre other = new Ogre(OGRE_X, OGRE_Y, "blue", random.nextInt(OGRE_INITIAL_LIFEPOINTS), random.nextInt(2 * OGRE_INITIAL_STRENGTH));
                // the other ogre randomly is given a companion
                if (random.nextBoolean()) {
                    other.meet(new Ogre(OGRE_X - 2 * OGRE_DIST, OGRE_Y, "yellow", OGRE_INITIAL_LIFEPOINTS, OGRE_INITIAL_STRENGTH));
                }
            ...
            case DRINK_ELIXIR:
                ...
                currentText = stringifyOgre(adventurer);
                currentText += "DRINKS (" + elixir.getCharacteristic() + ") -- ";
                break;
            default:
                currentText = stringifyOgre(adventurer);
                currentText += "NOTHING HAPPENS";
                break;
            }
            if (adventurer.getCompanion() != null) {
                adventurer.getCompanion().draw();
            }
            adventurer.draw();
    ...
    String stringifyOgre(Ogre ogre) {
        String s = "";
        s += "LP: " + ogre.getLifePoints() + ", ST: " + ogre.getStrength() + " -- ";
        Ogre companion = ogre.getCompanion();
        if (companion != null) {
            s += "Companion LP: " + companion.getLifePoints() + ", ST: " + companion.getStrength() + " -- ";
        } else {
            s += "No companion -- ";
        }
        return s;
